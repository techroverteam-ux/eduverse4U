...
export default function SuperAdminMasterPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const [students, teachers, classes, subjects] = await Promise.all([
        masterAPI.getStudents(schoolId).catch(() => []),
        masterAPI.getTeachers(schoolId).catch(() => []),
        masterAPI.getClasses(schoolId).catch(() => []),
        masterAPI.getSubjects(schoolId).catch(() => [])
      ])
  ...
  const downloadStudentsTemplate = async () => {
    try {
      ...
      toast.success('Students template downloaded')
    } catch (error) {
  ...
  const downloadTeachersTemplate = async () => {
    try {
      const blob = await masterAPI.getTeachersTemplate()
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      ...
      a.click()
      window.URL.revokeObjectURL(url)
      toast.success('Teachers template downloaded')
    } catch (error) {
      toast.error('Failed to download template')
    }
  }
  ...
  const handleBulkUploadStudents = () => {
    document.getElementById('bulk-upload-students')?.click()
  }

  const handleBulkUploadTeachers = () => {
    document.getElementById('bulk-upload-teachers')?.click()
  }
  ...
  const handleTeachersUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (!file) return

    try {
      const schoolId = localStorage.getItem('schoolId') || 'default-school'
      await masterAPI.bulkUploadTeachers(file, schoolId)
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Configure and manage all school master data across the platform</p>
          </div>
          <div className="flex space-x-3">
            <Button variant="outline" className="border-green-200 text-green-700 hover:bg-green-50" onClick={downloadStudentsTemplate}>
              <Download className="h-4 w-4 mr-2" />
              Templates
            </Button>
            <Button variant="outline" className="border-blue-200 text-blue-700 hover:bg-blue-50" onClick={handleBulkUploadStudents}>
              <Upload className="h-4 w-4 mr-2" />
              Bulk Upload
            </Button>
            <input
              id="bulk-upload-students"
              type="file"
              accept=".xlsx,.xls"
              className="hidden"
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Button className="bg-gradient-to-r from-blue-600 to-purple-600 h-12" onClick={handleBulkUploadStudents}>
              <Upload className="h-4 w-4 mr-2" />
              Bulk Upload Students
            </Button>
            <Button className="bg-gradient-to-r from-green-600 to-teal-600 h-12" onClick={handleBulkUploadTeachers}>
              <Upload className="h-4 w-4 mr-2" />
              Bulk Upload Teachers
            </Button>
            <Button className="bg-gradient-to-r from-orange-600 to-red-600 h-12" onClick={downloadStudentsTemplate}>
              <Download className="h-4 w-4 mr-2" />
              Download Templates
            </Button>
          </div>
